...
public class UserController {
	...
	public Map<String, Object> register(HttpServletRequest request,
		...
		// user校验合法
		User user0 = userService.queryUserByUserName(user.getUsername());// 看看邮箱有没有被用过
		if (user0 == null) { // new user
			user.setId(commonUtils.getUUID());

			user.setPassword(commonUtils.encodeByMd5(user.getPassword()));
			user.setCreateTime(new Date());
			user.setLastLoginTime(null);
			user.setStatus(0);
			user.setRandomCode(commonUtils.getUUID());
			if (userService.insertUser(user)) { // insert user success
//				// 发送激活邮件
//				mailUtils.sendActivateMail(user.getEmail(), user.getUsername(),
//						user.getRandomCode());
//				map.put("code", 0);
//				map.put("msg", "ok");
//				map.put("data", 0);
			...
			if (user0.getStatus() == 0) {// user not activate
				...
				user.setPassword(commonUtils.encodeByMd5(user.getPassword()));
				user.setCreateTime(user0.getCreateTime());
				user.setLastLoginTime(null);
				user.setStatus(0);
				user.setRandomCode(commonUtils.getUUID());
	...
	public Map<String, Object> login(HttpServletRequest request,
		...
		if (user0 == null) { // no this user
			...
			if (user0.getPassword().equals(
					commonUtils.encodeByMd5(user.getPassword()))) {// password
																	// is right
				map.put("code", 0);
				JSONObject json = new JSONObject();
