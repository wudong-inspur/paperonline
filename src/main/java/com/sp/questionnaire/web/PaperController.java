...
/**
 * description: Author:Xiaowanwan Date:2018/9/16-14:05
 */
@Controller
public class PaperController {
	...
	public Map<String, Object> paperLists(HttpServletRequest request)
		...
		for (Paper p : list) {// 遍历list，把Paper转换成PaperQueryView类型
			...
			paperQueryView.setCreateTime(commonUtils.getLongByDate(p
					.getCreateTime()));
			paperQueryView.setStartTime(commonUtils.getDateStringByDate(p
					.getStartTime()));
			paperQueryView.setEndTime(commonUtils.getDateStringByDate(p
					.getEndTime()));
	...
	public Map<String, Object> viewPaper(HttpServletRequest request,
			@RequestBody String id) throws ParseException {
		/*
		...
		if (id == null) {
			...
			if (p == null) {
				...
				for (Question q : list) {
					...
					viewPaperQuestion.setQuestionOption(JSONArray.fromObject(q
							.getQuestionOption()));
	 ...
	 *                       addPaper(HttpServletRequest request, @RequestBody
	 *                       AddPaperViewPaper paper) throws ParseException {
	 *                       <p>
	 *                       <p>
	 *                       User user = (User) request.getAttribute("admin");
	 *                       <p>
	 *                       //no need id for add new paper String id = null;
	 *                       PaperMethodHelp paperMethodHelp = new
	 *                       PaperMethodHelp(); //return updatePaper((request,
	 *                       paper)); return paperMethodHelp.insertPaper(paper,
	 *                       user.getId(), id);
	 *                       <p>
	 *                       }
	 */
	...
	public Map<String, Object> updatePaper(HttpServletRequest request,
			@RequestBody UpdatePaperViewPaper paper) throws ParseException {
		...
		if (paper.getId() == null) {
			// 插入代码
			map = paperMethodHelp.insertPaper(addPaperViewPaper, user.getId(),
					null);
			...
			if (user != null) {
				// 通过检测，准备添加数据
				if (paperService.updatePaperQuestions(paper, user.getId(),
						addPaperViewPaper)) {
					map.put("code", 0);
					map.put("msg", "ok");
					map.put("data", 0);
	...
	public Map<String, Object> dataPaper(HttpServletRequest request,
			@RequestBody String id) throws ParseException {
		JSONObject json = JSONObject.fromObject(id);
		id = json.getString("id");
		...
		if (id == null) {
			...
			if (paperService.dataPaper(id) == null) {
				...
				// DataPaperViewPaper dataPaperViewPaper = (DataPaperViewPaper)
